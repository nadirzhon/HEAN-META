...
def run_command(cmd: List[str], cwd: Optional[Path] = None, timeout: int = 30) -> tuple[str, str, int]:
...
def generate_reports():
    ...
    # 1. repo_tree.txt
    print("  → repo_tree.txt")
    stdout, stderr, code = run_command(["tree", "-L", "5", "-I", "__pycache__|*.pyc|.git"], timeout=10)
    if code == 0:
        with open(reports_dir / "repo_tree.txt", "w", encoding="utf-8") as f:
            f.write(stdout)
    else:
        # Fallback: use find
        stdout, _, _ = run_command(["find", ".", "-type", "d", "-not", "-path", "*/.git/*", "-not", "-path", "*/__pycache__/*"], timeout=10)
        with open(reports_dir / "repo_tree.txt", "w", encoding="utf-8") as f:
    ...
    with open(reports_dir / "git_status.txt", "w", encoding="utf-8") as f:
        if code == 0:
            ...
            f.write(f"SKIPPED: Git not available or not a git repository\nExit code: {code}\n{stderr}")
    
    # 4. git_log.txt
    print("  → git_log.txt")
    stdout, stderr, code = run_command(["git", "log", "-n", "200", "--oneline", "--decorate"], timeout=10)
    ...
    if code == 0:
        stdout, stderr, code = run_command(["ruff", "check", "--output-format=text", "src/"], timeout=30)
    ...
    # Find compose files
    compose_files = list(PROJECT_ROOT.glob("docker-compose*.yml")) + list(PROJECT_ROOT.glob("docker-compose*.yaml"))
    ...
    stdout, stderr, code = run_command(["python3", "-c", "import hean; print('HEAN module import: OK')"], timeout=10)
