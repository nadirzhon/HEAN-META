...
class BybitHTTPAdapter(BybitActions):
    ...
    async def get_min_notional(self, symbol: str) -> float:
        ...
        return rules.get("minNotional", 5.0)  # Default to 5 USD

    async def get_symbol_rules(
        self, symbol: str
    ) -> dict[str, Any]:
        """Get symbol rules from Bybit API (requires instrument info endpoint).

        Returns default values if API endpoint not available.
        """
        # Bybit API v5 has /v5/market/instruments-info endpoint
        # For now, return defaults based on common Bybit rules
        # Future: implement actual API call if endpoint available
        defaults = {
            "minQty": 0.001,
            "qtyStep": 0.001,
            "minNotional": 5.0,  # 5 USD minimum
            "pricePrecision": 2,
        }
        logger.warning(
            f"Using default symbol rules for {symbol}. "
            "Consider implementing /v5/market/instruments-info endpoint."
        )
        return defaults
