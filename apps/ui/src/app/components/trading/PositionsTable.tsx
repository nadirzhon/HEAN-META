...
export function PositionsTable({ positions, compact = true }: PositionsTableProps) {
  ...
  const tableContent = (
    <Table>
      ...
      <TableBody>
        {positions.length === 0 ? (
          <TableRow>
            <TableCell colSpan={compact ? 4 : 9} className="text-center text-muted-foreground py-8">
              No open positions
            </TableCell>
          </TableRow>
        ) : (
          positions.map((position) => (
            <TableRow key={position.id} className="border-border/50 hover:bg-muted/30">
              ...
              <TableCell>
                <span className={`font-mono text-xs ${getSideColor(position.side)}`}>{position.side}</span>
              </TableCell>
              {!compact && (
                <>
                  <TableCell className="text-right font-mono tabular-nums text-xs">${formatPrice(position.entryPrice)}</TableCell>
                  <TableCell className="text-right font-mono tabular-nums text-xs">${formatPrice(position.lastPrice)}</TableCell>
                </>
              )}
              <TableCell className={`text-right font-mono tabular-nums text-xs ${getPnLColor(position.unrealizedPnL)}`}>
          ...
          <div className="p-2 border-t border-border/50 flex justify-end">
            <Dialog open={expanded} onOpenChange={setExpanded}>
              ...
              <DialogContent className="max-w-[95vw] max-h-[90vh] overflow-auto">
                <DialogHeader>
                  <DialogTitle>Open Positions ({positions.length})</DialogTitle>
                </DialogHeader>
                <div className="rounded-lg border border-border bg-card/30 backdrop-blur-sm">
                  <Table>
                    <TableHeader>
