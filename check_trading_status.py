    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.get(
                f"{api_base}/engine/status", timeout=aiohttp.ClientTimeout(total=5)
            ) as resp:
                if resp.status == 200:
                    ...
                    if total_profit >= 0:
                        print(
                            f"  {Colors.GREEN}Общая прибыль: ${total_profit:,.2f} ({profit_pct:+.2f}%){Colors.RESET}"
                        )
                    else:
                        print(
                            f"  {Colors.RED}Общий убыток: ${total_profit:,.2f} ({profit_pct:.2f}%){Colors.RESET}"
                        )
                    ...
                    print(
                        f"{Colors.RED}✗ Ошибка получения статуса: HTTP {resp.status}{Colors.RESET}"
                    )
        ...
        try:
            async with session.get(
                f"{api_base}/orders/positions", timeout=aiohttp.ClientTimeout(total=5)
            ) as resp:
                if resp.status == 200:
                    ...
                    if positions and len(positions) > 0:
                        ...
                        for i, pos in enumerate(positions, 1):
                            ...
                            if unrealized_pnl >= 0:
                                print(
                                    f"     {Colors.GREEN}Нереализованная прибыль: ${unrealized_pnl:,.2f}{Colors.RESET}"
                                )
                            else:
                                print(
                                    f"     {Colors.RED}Нереализованный убыток: ${unrealized_pnl:,.2f}{Colors.RESET}"
                                )

                            if realized_pnl != 0:
                                if realized_pnl >= 0:
                                    print(
                                        f"     {Colors.GREEN}Реализованная прибыль: ${realized_pnl:,.2f}{Colors.RESET}"
                                    )
                                else:
                                    print(
                                        f"     {Colors.RED}Реализованный убыток: ${realized_pnl:,.2f}{Colors.RESET}"
                                    )
                        ...
                        if total_unrealized_pnl >= 0:
                            print(
                                f"    {Colors.GREEN}Общая нереализованная прибыль: ${total_unrealized_pnl:,.2f}{Colors.RESET}"
                            )
                        else:
                            print(
                                f"    {Colors.RED}Общий нереализованный убыток: ${total_unrealized_pnl:,.2f}{Colors.RESET}"
                            )

                        if total_realized_pnl != 0:
                            if total_realized_pnl >= 0:
                                print(
                                    f"    {Colors.GREEN}Общая реализованная прибыль: ${total_realized_pnl:,.2f}{Colors.RESET}"
                                )
                            else:
                                print(
                                    f"    {Colors.RED}Общий реализованный убыток: ${total_realized_pnl:,.2f}{Colors.RESET}"
                                )

                        total_pnl = total_unrealized_pnl + total_realized_pnl
                        if total_pnl >= 0:
                            print(
                                f"    {Colors.BOLD}{Colors.GREEN}ОБЩАЯ ПРИБЫЛЬ: ${total_pnl:,.2f}{Colors.RESET}"
                            )
                        else:
                            print(
                                f"    {Colors.BOLD}{Colors.RED}ОБЩИЙ УБЫТОК: ${total_pnl:,.2f}{Colors.RESET}"
                            )
                    else:
                        print(f"  {Colors.YELLOW}Нет открытых позиций{Colors.RESET}")
                else:
                    print(
                        f"{Colors.RED}✗ Ошибка получения позиций: HTTP {resp.status}{Colors.RESET}"
                    )
        except Exception as e:
        ...
        try:
            async with session.get(
                f"{api_base}/analytics/performance", timeout=aiohttp.ClientTimeout(total=5)
            ) as resp:
                if resp.status == 200:
                    ...
                    if strategies:
                        ...
                        for strategy_id, metrics in strategies.items():
                            ...
                            if profit_factor >= 1.0:
                                print(
                                    f"    {Colors.GREEN}Profit Factor: {profit_factor:.2f}{Colors.RESET}"
                                )
                            else:
                                print(
                                    f"    {Colors.RED}Profit Factor: {profit_factor:.2f}{Colors.RESET}"
                                )
                    ...
                    print(
                        f"{Colors.RED}✗ Ошибка получения статистики: HTTP {resp.status}{Colors.RESET}"
                    )
        except Exception as e:
        ...
        try:
            async with session.get(
                f"{api_base}/orders?status=all", timeout=aiohttp.ClientTimeout(total=5)
            ) as resp:
                if resp.status == 200:
                    ...
                    if orders:
                        ...
                        open_count = sum(
                            1 for o in orders if o.get("status") in ["pending", "placed"]
                        )
