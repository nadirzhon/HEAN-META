...
async def check_api_endpoint(session: aiohttp.ClientSession, base_url: str, endpoint: str) -> dict[str, Any]:
...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if engine_status["response"]:
            ...
            print_status(
                "No Undefined Values",
                not has_undefined,
                f"{len(undefined_fields)} undefined fields found" if has_undefined else "All fields have values"
            )
            if undefined_fields:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if engine_status["response"]:
            ...
            if debug_mode and paper_trade_assist:
                ...
                print_status(
                    "Warden Filter Mode",
                    True,
                    f"{mode_status} - Filters loosened for testing"
                )
                print(f"  DEBUG_MODE: {debug_mode}")
                print(f"  PAPER_TRADE_ASSIST: {paper_trade_assist}")
                print(f"  {Colors.YELLOW}⚠️  WARNING: Aggressive Mode is for testing only!{Colors.RESET}")
            elif debug_mode:
                print_status(
                    "Warden Filter Mode",
                    True,
            ...
            if api_key:
                ...
                if api_key and len(api_key) > 10:
                    print_status(
                        "API Key Configured",
                        True,
                        f"Key present (length: {len(api_key)} chars)"
                    )
                    print(f"  {Colors.YELLOW}⚠️  Verify 'Trade' permission is active in Bybit API settings{Colors.RESET}")
                else:
                    print_status(
                        "API Key Configured",
                        False,
                ...
                print_status(
                    "API Key Configured",
                    False,
                    "BYBIT_API_KEY not set (paper mode only)"
                )
        except Exception as e:
            print_status(
                "API Key Check",
        ...
        strategies = await check_api_endpoint(session, api_base, "/strategies")
        if strategies["response"]:
            strategies_list = strategies["response"] if isinstance(strategies["response"], list) else []
            print_status(
                "Active Strategies",
            ...
            for strat in strategies_list[:5]:  # Show first 5
                ...
                signal_strength = "N/A"
                try:
                    analytics = await check_api_endpoint(session, api_base, f"/analytics/performance")
                    if analytics["response"]:
                        ...
                        if strat_id in strategies_metrics:
                            ...
                            if total > 0:
                                ...
                                signal_strength = f"{strength_pct:.1f}% pass rate ({signals}/{total})"
                except:
                    pass
                
                print(f"  {color}{status}{Colors.RESET}: {strat_id} | Signal Strength: {signal_strength}")
        ...
        try:
            ...
            if analytics["response"]:
                ...
                if total_generated > 0:
                    ...
                    print(f"    Accepted: {total_generated - total_rejected} ({100 - rejection_rate:.1f}%)")
        except:
            pass
