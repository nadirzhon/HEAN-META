...
async def diagnose():
    ...
    if filled:
        ...
        for o in filled[-5:]:
            print(f"  {o.get('symbol')} {o.get('side')} | {o.get('strategy_id')} | {o.get('timestamp')}")
        print()
    ...
    if summary.pipeline_counters:
        for counter, count in sorted(summary.pipeline_counters.items(), key=lambda x: x[1], reverse=True):
