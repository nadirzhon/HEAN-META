...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate trading agents using LLM prompts")
    ...
    try:
        if args.prompt_type == "initial":
            if args.count > 1:
                ...
                codes = generator.generate_initial_agents(count=args.count, output_dir=output_dir)
                ...
                code = generator.generate_agent(prompt_type="initial", output_path=args.output)
                if not args.output:
                    print(code)
            ...
            if not all(
                ...
                parser.error(
                    "Evolution requires --best-agents, --worst-agents, --market-conditions, --performance-metrics"
                )
            ...
            if not all(
                ...
                parser.error(
                    "Mutation requires --agent-code, --profit-factor, --total-pnl, --max-drawdown, --win-rate, --issues"
                )
            ...
            required = [
                ...
                args.volatility_value,
                args.trend_direction,
                args.trend_strength,
                args.volume_level,
                args.market_regime,
                args.spread_bps,
                args.historical_summary,
                args.suggested_style,
                args.suggested_timeframe,
                args.suggested_size,
                args.risk_approach,
            ]
            if not all(required):
            ...
            if not all(
                [args.agent_code, args.pf, args.pnl, args.dd, args.wr, args.sharpe, args.trades]
            ):
            ...
            code = generator.generate_agent(prompt_type="analytical", output_path=args.output)
            if not args.output:
                print(code)
