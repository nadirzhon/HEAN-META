...
async def get_trading_report() -> None:
    ...
    try:
        ...
        if filled_orders:
            ...
            for order in filled_orders:
                ...
                strategy_stats[strategy]["total_volume"] += order.get("size", 0) * order.get(
                    "price", 0
                )
            ...
            for strategy, stats in strategy_stats.items():
                print(f"   {strategy}:")
                print(
                    f"      Ордеров: {stats['count']} (Buy: {stats['buy']}, Sell: {stats['sell']})"
                )
            ...
            print()
            print(
                f"{'Символ':<12} {'Сторона':<8} {'Размер':<15} {'Цена':<15} {'Стратегия':<20} {'Время':<20}"
            )
            print("-" * 100)
            ...
            for order in filled_orders[-10:]:
                ...
                print(
                    f"{symbol:<12} {side:<8} {size:<15.6f} ${price:<14,.2f} {strategy:<20} {time_str:<20}"
                )
        ...
        if pending_orders:
            ...
            for order in pending_orders[:5]:
                print(
                    f"   {order.get('symbol')} {order.get('side').upper()} | Size: {order.get('size', 0):.6f} | Status: {order.get('status')}"
                )
    ...
    try:
        ...
        if positions:
            ...
            print(
                f"{'Символ':<12} {'Сторона':<8} {'Размер':<15} {'Entry':<15} {'Mark':<15} {'PnL':<15}"
            )
            print("-" * 100)
            ...
            for pos in positions:
                ...
                print(
                    f"{symbol:<12} {side:<8} {size:<15.6f} ${entry_price:<14,.2f} ${mark_price:<14,.2f} ${pnl_sign}{unrealized_pnl:<14,.2f}"
                )
