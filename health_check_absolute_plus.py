...
class AbsolutePlusHealthCheck:
    ...
    def check_cpp_dependencies(self) -> HealthCheckResult:
        ...
        try:
            ...
            if simdjson_found and boost_found:
                ...
                if not uws_found:
                    result.details["Note"] = (
                        "uWebSockets not linked (WebServer may use FastAPI instead)"
                    )
            else:
    ...
    async def check_web_server(self) -> HealthCheckResult:
        ...
        try:
            ...
            try:
                ...
                result.details["Note"] = (
                    "Start server with: make dev or docker-compose up -d --build"
                )
    ...
    async def check_logic_validation(self) -> HealthCheckResult:
        ...
        try:
            ...
            if high_probability_detections:
                result.passed = True
                result.message = (
                    f"Detected {len(high_probability_detections)} high-probability signals (90%+)"
                )
                ...
                result.details["Note"] = (
                    "This is normal if market conditions don't warrant high-confidence trades"
                )
    ...
    async def check_self_healing(self) -> HealthCheckResult:
        ...
        try:
            ...
            result.details["Average API Latency"] = (
                f"{health_after_jitter.get('api_latency_ms', 0):.2f}ms"
            )
            ...
            if health_after_jitter["status"] in ["degraded", "critical"]:
                ...
                result.details["Note"] = (
                    "System may not have processed jitter yet (monitoring interval)"
                )
