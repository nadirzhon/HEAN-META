...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, connection_id: str) -> None:
        ...
        logger.info(
            f"✅ Sent initial state to {connection_id}: Engine={'RUNNING' if engine_running else 'STOPPED'}, Redis={redis_status.upper()}, Equity=${equity:.2f}"
        )
...
async def update_trading_state_cache(
    ...
    async with trading_state_lock:
        ...
        if positions is not None:
            ...
            existing = {
                p.get("position_id"): p
                for p in trading_state_cache.get("positions", [])
    ...
    if redis_state_manager:
        try:
            ...
            if orders is not None:
                await redis_state_manager.set_state_atomic("orders", orders, namespace="state")
            if positions is not None:
                await redis_state_manager.set_state_atomic(
...
async def sync_trading_state_from_engine(
    reason: str = "manual", full_snapshot: bool = True
) -> None:
    ...
    try:
        ...
        await broadcast_trading_state(
            account_state, positions, orders if full_snapshot else None, full_snapshot=full_snapshot
        )
...
async def lifespan(app: FastAPI):
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                logger.warning(
                    f"REDIS_CHECK_RETRY {attempt + 1}/3 failed: {e}. Retrying...", exc_info=True
                )
...
async def forward_events_to_topics():
    ...
    async def handle_candle(event: Event) -> None:
        ...
        candle_payload = {
            "kind": "kline",
            **(await market_data_store.record_candle(timeframe, candle)),
        }
        await emit_topic_event(
            topic="market_data",
    ...
    async def handle_signal(event: Event) -> None:
        ...
        if signal is not None:
            ...
            signal_id = (
                ...
                or f"{signal.strategy_id}:{signal.symbol}:{signal.side}:{uuid.uuid4().hex[:8]}"
            )
            ...
            features = {
                ...
                if k not in ("signal_id", "confidence", "score", "signal_type", "type")
            }
        else:
            payload = {
                "type": "signal",
            ...
            signal_id = (
                metadata.get("signal_id")
                or f"{payload.get('strategy_id')}:{payload.get('symbol')}:{uuid.uuid4().hex[:8]}"
            )
            confidence = metadata.get("confidence") or metadata.get("score")
            signal_type = metadata.get("signal_type") or metadata.get("type")
            features = {
    ...
    async def handle_pnl_update(event: Event) -> None:
        ...
        if now - last_account_broadcast >= 2.0:  # Throttle to every 2 seconds
            ...
            pnl_snapshot_payload = {
                ...
                "free_margin": account_state.get(
                    "available_balance", account_state.get("free_margin", 0.0)
                ),
            }
            await emit_topic_event(
                topic="trading_events",
    ...
    async def handle_position_event(event: Event) -> None:
        ...
        position_update_payload = {
            ...
            "realized_pnl_delta": payload.get("realized_pnl", 0.0)
            if event.event_type == EventType.POSITION_CLOSED
            else None,
    ...
    async def handle_order_decision(event: Event) -> None:
        ...
        decision_payload = {
            ...
            "gating": {
                "risk_ok": decision.get("reason_code")
                not in ("RISK_BLOCK", "DEPOSIT_PROTECTION", "PROTECTION_BLOCK"),
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    logger.info(
        f"[HEAN API] WebSocket connection attempt from {client_host}, connection_id: {connection_id}"
    )
        ...
        while True:
            try:
                ...
                if time.time() - last_ping_time > CONNECTION_TIMEOUT:
                    logger.warning(
                        f"WebSocket connection {connection_id} timed out (no activity for {CONNECTION_TIMEOUT}s)"
                    )
                    break
...
async def trigger_killswitch(request: Request) -> Dict[str, Any]:
    ...
    if not bus:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Event bus not initialized"
        )
    ...
    try:
        ...
        logger.critical(
            f"EMERGENCY KILLSWITCH TRIGGERED via API: {reason} ({response_time_ms:.2f}ms)"
        )
