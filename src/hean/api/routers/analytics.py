    ...
    try:
        ...
        for strategy_id, metrics_data in strategy_metrics.items():
            ...
            if avg_loss > 0:
                ...
                kelly_fraction = (
                    (win_rate * odds_ratio - (1.0 - win_rate)) / odds_ratio
                    if odds_ratio > 0
                    else 0.0
                )
        ...
        return {"strategies": strategies_data, "curve_points": curve_points}
    except Exception as e:
...
async def get_safety_net_status(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        if not trading_system or not hasattr(trading_system, "_safety_net"):
            return {"active": False, "entropy_metrics": {}, "hedge_positions": {}}
