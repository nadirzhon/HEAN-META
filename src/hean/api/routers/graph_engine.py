...
async def get_graph_state(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        for symbol in symbols:
            asset_data = {"symbol": symbol, "price": 0.0, "correlation": {}, "leader_score": 0.0}
            ...
            if hasattr(graph_engine, "get_correlation"):
                asset_data["leader_score"] = (
                    graph_engine.get_lead_lag("BTCUSDT", symbol) if symbol != "BTCUSDT" else 1.0
                )
        ...
        return {"assets": assets, "correlations": correlations, "asset_count": len(assets)}
...
async def get_leader(request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        return {"leader": leader, "leader_score": leader_score}
    except Exception as e:
