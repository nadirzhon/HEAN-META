...
async def get_stats(request: Request):
    ...
    if not engine_facade or not hasattr(engine_facade, "_multimodal_swarm"):
        return {"tensor_size": 18, "modality_weights": {}, "num_agents": 0}
...
async def get_tensors(request: Request, symbol: str, limit: int = 10):
    ...
    return [
        MultimodalTensorResponse(
            ...
            symbol=t.symbol,
            price_features=t.price_features.tolist()
            if isinstance(t.price_features, np.ndarray)
            else list(t.price_features),
            sentiment_features=t.sentiment_features.tolist()
            if isinstance(t.sentiment_features, np.ndarray)
            else list(t.sentiment_features),
            onchain_features=t.onchain_features.tolist()
            if isinstance(t.onchain_features, np.ndarray)
            else list(t.onchain_features),
            macro_features=t.macro_features.tolist()
            if isinstance(t.macro_features, np.ndarray)
            else list(t.macro_features),
            unified_tensor=t.unified_tensor.tolist()
            if isinstance(t.unified_tensor, np.ndarray)
            else list(t.unified_tensor),
...
async def get_modality_weights(request: Request):
    ...
    if not engine_facade or not hasattr(engine_facade, "_multimodal_swarm"):
        return {"price": 0.4, "sentiment": 0.2, "onchain": 0.25, "macro": 0.15}
