...
class TradingMetrics:
    ...
    def _count_in_window(
        self, buffer: deque[tuple[float, int | float]], window_seconds: int
    ) -> int | float:
    ...
    async def get_metrics(self) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            top_reasons = [
                {
                    "code": code,
                    "count": count,
                    "pct": round(count / max(len(recent_reasons), 1) * 100, 1),
                }
            ...
            return {
                ...
                "last_signal_ts": datetime.fromtimestamp(
                    self._last_signal_ts, tz=timezone.utc
                ).isoformat()
                if self._last_signal_ts
                else None,
                "last_order_ts": datetime.fromtimestamp(
                    self._last_order_ts, tz=timezone.utc
                ).isoformat()
                if self._last_order_ts
                else None,
                "last_fill_ts": datetime.fromtimestamp(
                    self._last_fill_ts, tz=timezone.utc
                ).isoformat()
