...
class CausalDiscoveryEngine:
    ...
    def _calculate_granger_causality(
        self, asset_a: str, asset_b: str, max_lag: int = 50
            ...
            try:
                ...
                for asset_a in self._symbols:
                    for asset_b in self._symbols:
                        ...
                        if (
                            asset_a in self._timestamp_history
                            and len(self._timestamp_history[asset_a]) > 1
                        ):
        ...
        for target_asset in self._causal_graph.successors(tick.symbol):
            ...
            await self._bus.publish(
                Event(
                    ...
                    data={"domino_prediction": prediction, "type": "causal_domino"},
                )
            )

            logger.info(
                f"Domino prediction: {tick.symbol} -> {target_asset} "
                    ...
                    if current_time >= prediction.predicted_time:
                        ...
                        if prediction.affected_asset in self._price_history:
                            ...
                            if len(prices) >= 2:
                                ...
                                logger.debug(
                                    f"Validated prediction: {prediction.trigger_asset} -> "
