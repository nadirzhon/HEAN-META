...
class GrangerCausalityCalculator:
    ...
    @staticmethod
    def calculate(X: np.ndarray, Y: np.ndarray, max_lag: int = 10) -> Tuple[float, float, int]:
        """
...
class TransferEntropyCalculator:
    ...
    @staticmethod
    def calculate(X: np.ndarray, Y: np.ndarray, lag: int = 1, bins: int = 10) -> float:
        """
        ...
        try:
            ...
            if lag > 0:
                ...
                Y_prev = (
                    Y_binned[lag - 1 : -1] if lag > 0 and lag < len(Y_binned) else Y_binned[:-1]
                )
...
class CausalInferenceEngine:
    ...
    async def _update_causal_relationships(self) -> None:
        ...
        for target in self._target_symbols:
            for source in self._source_symbols:
                ...
                is_significant = (
                    f_stat >= self._min_causality_threshold
                    and te >= self._min_transfer_entropy
                    and p_value < 0.05
                )
    ...
    async def _detect_pre_echoes(self) -> None:
        ...
        for (source, target), relationship in list(self._causal_relationships.items()):
            ...
            recent_source_return = (
                source_returns[-relationship.lag_period - 1]
                if len(source_returns) > relationship.lag_period
                else 0.0
            )
    ...
    def remove_source_symbol(self, symbol: str) -> None:
        ...
        if symbol in self._source_symbols:
            ...
            keys_to_remove = [(s, t) for (s, t) in self._causal_relationships.keys() if s == symbol]
