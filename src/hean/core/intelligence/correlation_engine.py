...
class CorrelationEngine:
    ...
    def _identify_price_gap(self, symbol_a: str, symbol_b: str) -> tuple[float, str, str] | None:
        ...
        if len(self._price_history[symbol_a]) < 50 or len(self._price_history[symbol_b]) < 50:
            return None
        ...
        ratios = [
            prices_a[-min_len + i] / prices_b[-min_len + i] if prices_b[-min_len + i] > 0 else 1.0
    ...
    async def _emit_pair_signal(self, laggard: str, leader: str, z_score: float) -> None:
        ...
        """
        # Get current prices
        if len(self._price_history[laggard]) == 0 or len(self._price_history[leader]) == 0:
            return
        ...
        # Publish signals
        await self._bus.publish(Event(event_type=EventType.SIGNAL, data={"signal": long_signal}))

        await self._bus.publish(Event(event_type=EventType.SIGNAL, data={"signal": short_signal}))
