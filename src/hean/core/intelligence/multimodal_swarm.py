...
class MultimodalSwarm:
    ...
    def __init__(
        self, bus: EventBus, symbols: List[str] = None, window_size: int = 100, num_agents: int = 50
    ):
    ...
    async def _update_tensor(
        ...
        if current_tensors and current_tensors[-1].timestamp > datetime.utcnow() - timedelta(
            seconds=5
        ):
        ...
        while self._running:
            ...
            for symbol in self._symbols:
                ...
                if (
                    ...
                    or sentiment_features is not None
                    or onchain_features is not None
                    or macro_features is not None
                ):
        ...
        while self._running:
            ...
            for symbol in self._symbols:
                ...
                for agent in self._agents:
                    ...
                    agent["confidence"] = (
                        np.mean(list(agent["performance_history"]))
                ...
                if total_votes > 0:
                    ...
                    execution_signal_strength = avg_confidence * max(
                    ...
                    if buy_percentage > 0.6 and avg_confidence > 0.6:
                        ...
                        await self._bus.publish(
                            Event(event_type=EventType.SIGNAL, data={"signal": signal})
                        )
    ...
    def _agent_analyze(self, agent: Dict[str, Any], tensor: MultimodalTensor) -> Tuple[str, float]:
        """Agent analyzes tensor based on specialization."""
