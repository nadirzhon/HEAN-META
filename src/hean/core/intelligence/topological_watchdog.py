...
class TopologicalWatchdog:
        ...
        if is_disconnected:
            ...
            if not self._halt_active:
                ...
                disconnected_duration = (
                    (now - self._disconnected_since).total_seconds()
    ...
    async def _trigger_halt(self, reason: str) -> None:
        ...
        logger.critical(f"🚨 TOPOLOGICAL WATCHDOG: TRADING HALTED - {reason}")
    ...
    async def _clear_halt(self) -> None:
        ...
        logger.info("✅ TOPOLOGICAL WATCHDOG: Trading halt cleared. Market manifold reconnected.")
