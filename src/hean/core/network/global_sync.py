...
class NodeState:
    ...
    def update_heartbeat(
        ...
        self.takeover_capable = (
            is_healthy and cpu_usage < 0.8 and memory_usage < 0.85 and len(exchange_latencies) > 0
        )
...
class DistributedNodeManager:
    ...
    async def _master_check_loop(self) -> None:
        ...
        while self._running:
            try:
                await asyncio.sleep(
                    self._heartbeat_timeout_ms / 1000.0 / 2.0
                )  # Check at 2x heartbeat rate
                ...
                if self._master_node and self._master_node != self._local_region:
                    ...
                    if master and not master.is_alive(self._heartbeat_timeout_ms):
                        logger.warning(
                            f"Master node {self._master_node.name} appears offline, initiating failover"
                        )
    ...
    async def _determine_master_node(self) -> None:
        ...
        if best_node.region != self._master_node:
            if best_node.region == self._local_region:
                ...
                if self._master_node and self._master_node != self._local_region:
                    logger.info(
                        f"Local node should become master (current: {self._master_node.name})"
                    )
            else:
    ...
    async def _request_master_role(self) -> None:
        ...
        logger.warning(
            f"MASTER ROLE TAKEOVER: {self._local_region.name} is now master (failover #{self._failover_count})"
        )
    ...
    def get_node_stats(self) -> Dict:
        ...
        return {
            ...
            "execution_count": {
                region.name: count for region, count in self._execution_count.items()
            },
