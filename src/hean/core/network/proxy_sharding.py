...
class ProxyState:
    ...
    def update_request(self, success: bool = True, latency_ms: Optional[float] = None) -> None:
        ...
        if success:
            ...
            if latency_ms is not None:
                ...
                self.latency_ms = (
                    statistics.mean(self.recent_latencies)
    ...
    def get_score(self) -> float:
        ...
        success_rate = (
            (total_requests - self.error_count) / total_requests if total_requests > 0 else 0.5
        )

        score = latency_score * 0.3 + rate_score * 0.3 + health_score * 0.2 + success_rate * 0.2
    ...
    def _should_rotate(self) -> bool:
        ...
        if self._active_proxy_id:
            ...
            if proxy_state and proxy_state.rate_limit_usage >= 0.8:  # 80% threshold
                logger.warning(
                    f"Current proxy {self._active_proxy_id} rate limit at {proxy_state.rate_limit_usage:.2%}, rotating"
                )
                return True
        ...
        for proxy_id, state in self._proxy_states.items():
            if state.proxy.enabled:
                ...
                if state.consecutive_failures >= 3:
                    ...
                    logger.warning(
                        f"Proxy {proxy_id} marked as unhealthy (consecutive_failures={state.consecutive_failures})"
                    )
