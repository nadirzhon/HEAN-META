...
class DynamicAPIScouter:
    ...
    async def start(self) -> None:
        ...
        logger.info(
            f"Dynamic API Scouter started (check_interval={self._check_interval}s, switch_interval={self._switch_interval}s)"
        )
        ...
        try:
            ...
            try:
                await asyncio.wait_for(
                    asyncio.get_event_loop().sock_connect(sock, sockaddr), timeout=2.0
                )
        ...
        while self._running:
            try:
                ...
                if top_nodes:
                    ...
                    should_switch = (
                        self._current_fastest is None
                        or self._current_fastest.host != fastest.host
                        or (current_time - self._last_switch_time) >= self._switch_interval
                    )
                        ...
                        if self._endpoint_change_callback:
                            try:
                                await self._endpoint_change_callback(
                                    fastest.ws_url, fastest.rest_url
                                )
                            except Exception as e:
    ...
    def get_top_nodes(self, n: int = 5) -> List[NodeLatency]:
        ...
        """
        sorted_nodes = sorted(
            self._nodes, key=lambda node: (node.get_avg_latency(), -node.get_reliability())
        )
