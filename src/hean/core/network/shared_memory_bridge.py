        ...
        try:
            ...
            try:
                self._mutex = posix_ipc.Semaphore(
                    self._mutex_name, flags=posix_ipc.O_CREAT, initial_value=1
                )  # type: ignore
            ...
            logger.error(
                f"Shared memory {self._shm_name} does not exist. Is C++ Feed Handler running?"
            )
            return False
        ...
        while self._read_index < write_index and ticks_read < max_ticks:
            ...
            if tick:
                ...
                if self._last_sequence_id > 0 and tick.sequence_id != self._last_sequence_id + 1:
                    ...
                    if gap > 0:
                        logger.warning(
                            f"Sequence gap detected: missed {gap} ticks (expected {self._last_sequence_id + 1}, got {tick.sequence_id})"
                        )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "buffer_utilization": min(
                100.0, ((write_index - self._read_index) / self.RING_SIZE) * 100.0
            ),
        }
