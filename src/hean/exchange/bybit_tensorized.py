...
class BybitTensorizedMonitor:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{proxy.endpoint}/v5/public/time", timeout=aiohttp.ClientTimeout(total=5)
                ) as resp:
                    if resp.status == 200:
        ...
        while self.running:
            try:
                ...
                if new_proxy and new_proxy != self.current_proxy:
                    logger.info(
                        f"Switching to fastest proxy: {new_proxy.name} "
    ...
    async def _initialize_tensor(self, symbols: List[str]):
        ...
        num_features = (
            8  # price, volume, bid_volume, ask_volume, spread, imbalance, volatility, momentum
        )
    ...
    def _update_tensor(self, symbol: str, update: Dict):
        ...
        # Rotate tensor (FIFO)
        self.orderflow_tensor.data[symbol_idx, :-1, :] = self.orderflow_tensor.data[
            symbol_idx, 1:, :
        ]
