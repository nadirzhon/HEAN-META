...
class SmartLimitExecutor:
    ...
    async def place_post_only_order(
        self, order_request: OrderRequest, ofi_aggression: float = 0.0
    ) -> Order:
        ...
        predicted_slippage = self.predict_slippage(symbol, size, is_buy=(side == "buy"))
    ...
    async def place_post_only_order_with_jitter(
        ...
        for i in range(self._jitter_order_count):
            ...
            child_request = OrderRequest(
                ...
                stop_loss=order_request.stop_loss,
                take_profit=order_request.take_profit,
                signal_id=f"{order_request.signal_id}_jitter_{i}"
                if order_request.signal_id
                else None,
                metadata={
            ...
            if i < self._jitter_order_count - 1:
                delay_ms = random.uniform(self._jitter_delay_min_ms, self._jitter_delay_max_ms)
