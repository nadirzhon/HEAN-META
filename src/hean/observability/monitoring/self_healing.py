...
class SelfHealingMiddleware:
    ...
    async def _check_api_latency(self) -> None:
        ...
        if avg_latency > self._max_api_latency_ms:
            logger.warning(f"High API latency: avg={avg_latency:.2f}ms, max={max_latency:.2f}ms")
    ...
    async def _check_order_latency(self) -> None:
        ...
        if avg_latency > self._max_order_latency_ms:
            logger.warning(f"High order latency: avg={avg_latency:.2f}ms, max={max_latency:.2f}ms")
    ...
    async def _update_health_status(self) -> None:
        ...
        if (
            len(self._gc_collection_times) > 0
            and max(self._gc_collection_times) > self._max_gc_time_ms
        ):
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        avg_memory = (
            sum(self._memory_history) / len(self._memory_history) if self._memory_history else 0.0
        )
        avg_api_latency = (
            sum(self._api_latencies) / len(self._api_latencies) if self._api_latencies else 0.0
        )
        avg_gc_time = (
            sum(self._gc_collection_times) / len(self._gc_collection_times)
            if self._gc_collection_times
            else 0.0
        )
