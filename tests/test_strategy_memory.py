...
def test_memory_recovers_after_cooldown() -> None:
    ...
    assert not memory.is_in_cooldown("strategy_a"), "Expected cooldown to be expired"
...
def test_rolling_pf_calculation() -> None:
    ...
    assert abs(pf - expected_pf) < 0.01, f"Expected PF ≈ {expected_pf}, got {pf}"
...
def test_min_max_weight_bounds_respected() -> None:
    ...
    for strategy_id, weight in new_weights.items():
        assert 0.0 <= weight <= 1.0, f"Weight for {strategy_id} is {weight}, out of bounds"
